...
def Keplerian_elements(Dec, t, mu=1, par=0):
    ...
    r = np.sqrt(Dec[0]*Dec[0]+Dec[1]*Dec[1]+Dec[2]*Dec[2])
    V = np.sqrt(Dec[3]*Dec[3]+Dec[4]*Dec[4]+Dec[5]*Dec[5])
    ...
    Cxy = np.sqrt(c[0]*c[0]+c[1]*c[1])
    C = np.sqrt(Cxy*Cxy+c[2]*c[2])
    ...
    F = np.sqrt(f[0]*f[0]+f[1]*f[1]+f[2]*f[2])
    ...
    if np.abs(cosPeri) > 1:
        cosPeri = cosPeri/np.abs(cosPeri)
